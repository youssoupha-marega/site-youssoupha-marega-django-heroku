...
class ServiceMetadataTest(TestCase):
    ...
    def test_home_page_renders_metadata(self):
        ...
        # Afficher un extrait du HTML autour du service pour déboguer
        if "Service Test" in content:
            idx = content.find("Service Test")
            print(f"\n=== Extrait HTML autour du service (500 caractères avant et après) ===")
            excerpt = content[max(0, idx-500):min(len(content), idx+500)]
            print(excerpt)
    ...
    def test_debug_template_variables(self):
        """Déboguer les variables passées au template section_services.html"""
        from django.template.loader import get_template
        
        print("\n=== TEST 5: Débogage des variables du template ===")
        print(f"services_show_author_home: {self.profile.services_show_author_home}")
        print(f"services_show_profession_home: {self.profile.services_show_profession_home}")
        print(f"services_show_publish_date_home: {self.profile.services_show_publish_date_home}")
        print(f"services_show_update_date_home: {self.profile.services_show_update_date_home}")
        
        # Simuler le contexte passé au template
        context = {
            'site_profile': self.profile,
            'services': [self.service],
        }
        
        # Charger et rendre le template section_services.html
        template = get_template('includes/section_services.html')
        html = template.render(context)
        
        print(f"\n=== HTML rendu (section services) ===")
        print(html[:1500])
        
        print(f"\nHTML contient 'Youssoupha Marega': {'Youssoupha Marega' in html}")
        print(f"HTML contient 'Data Scientist': {'Data Scientist' in html}")
